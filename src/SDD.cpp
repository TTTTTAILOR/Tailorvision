 ...
 *                    基本汉化网页配置页面
 *            V1.4.3  修复休眠BUG，以实现串口实时更改地址，且缺少数据类型转换的问题
 * 
 * 引 脚 分 配： SCK  GPIO14  D5
 *             MOSI  GPIO13 D7
 *             RES   GPIO2  D4
 *             DC    GPIO0  D3
 *             LCDBL GPIO5  D1
...
String cityCode = "101270101";  //天气城市代码，成都:101270101
int tempnum = 0;   //温度百分比
int huminum = 0;   //湿度百分比
...
void Serial_set()
{
  ...
  if(Serial.available()>0)
  {
    ...
    {
      int CityCODE = 0;
      int CityC = atoi(incomingByte.c_str());//int n = atoi(xxx.c_str());//String转int以便存储到rom中，每次存两位数，9位数需要存5次，其实int类型支持0-255,但是存三位数时容易出现超过255的情况，因此保险只存两位
      Serial.println(CityC);
      if(CityC>=101000000 && CityC<=102000000 || CityC == 0)
      {
        for(int cnum=0;cnum<5;cnum++)
        {
          EEPROM.write(CC_addr+cnum,CityC%100);//城市地址写入城市代码，此处转换为Int类型是为节省rom空间，每次保存最后两位
          EEPROM.commit();//保存更改的数据
          CityC = CityC/100;//去掉最后两位
          delay(5);
        }
...
void LCD_reflash(int en)
{
  ...
  if(millis() - weaterTime > (60000*updateweater_time) || en == 1 || UpdateWeater_en != 0){ 
    if(WiFi.status() == WL_CONNECTED)
    {
      ...
      weaterTime = millis();
      while(!getNtpTime());//获取时间成功则继续执行代码，反之则进行死循环
    }
  }
}
...
void setup()
{
 ...
 //从eeprom读取背光亮度设置
  ...
  readwificonfig();//读取存储的wifi信息，并开始连接
  ...
  //当系统识别到当前网络不可用时，则开启WM配网入口，此时是如何将程序阻塞在这儿的？
  while (WiFi.status() != WL_CONNECTED) 
  {
    loading(30);  
    if(loadNum>=194)//走满到194步时还没连上则开启
    {
